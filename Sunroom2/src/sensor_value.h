...
struct SensorValue {
     ...
     */
    SensorValue(float val) : type(FLOAT) { 
        value.f = val; 
    }
     ...
     */
    SensorValue(const char* val) : type(STRING) { 
     ...
     */
    SensorValue(const std::string& val) : type(STRING) { 
    ...
    SensorValue(const SensorValue& other) : type(other.type) {
        switch (type) {
            case FLOAT: value.f = other.value.f; break;
            case INT: value.i = other.value.i; break;
            case STRING: new(&value.s) std::string(other.value.s); break;
        }
    }
