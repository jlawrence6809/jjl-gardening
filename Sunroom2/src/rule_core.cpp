...
RuleReturn processRuleCore(JsonVariantConst doc, const RuleCoreEnv &env)
{
    ...
    else if (std::strcmp(type, "EQ") == 0 || std::strcmp(type, "NE") == 0 || std::strcmp(type, "GT") == 0 || 
             std::strcmp(type, "LT") == 0 || std::strcmp(type, "GTE") == 0 || std::strcmp(type, "LTE") == 0)
    {
        RuleReturn a = processRuleCore(array[1], env);
        RuleReturn b = processRuleCore(array[2], env);
        if (a.type == ERROR_TYPE) return a;
        ...
        bool res = false;
        if (std::strcmp(type, "EQ") == 0) res = a.val == b.val;          // Equal
        else if (std::strcmp(type, "NE") == 0) res = a.val != b.val;     // Not equal
        else if (std::strcmp(type, "GT") == 0) res = a.val > b.val;      // Greater than
        else if (std::strcmp(type, "LT") == 0) res = a.val < b.val;      // Less than
        else if (std::strcmp(type, "GTE") == 0) res = a.val >= b.val;    // Greater than or equal
        else if (std::strcmp(type, "LTE") == 0) res = a.val <= b.val;    // Less than or equal
