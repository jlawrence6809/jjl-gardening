 ...
 */
using FunctionHandler = std::function<UnifiedValue(JsonArrayConst, const NewRuleCoreEnv &)>;
...
struct NewRuleCoreEnv {
     ...
     *     registry["GT"] = [](JsonArrayConst args, const NewRuleCoreEnv& env) {
 ...
 * ```
 */
UnifiedValue processNewRuleCore(JsonVariantConst doc, const NewRuleCoreEnv &env);
 ...
 */
void processNewRuleSet(const std::string rules[], int ruleCount, const NewRuleCoreEnv &env);
