...
void setRelays(AsyncWebServerRequest *request) {
    for (int i = 0; i < RUNTIME_RELAY_COUNT; i++) {
        ...
        if (request->hasParam(relayParam, POST_PARAM)) {
            RELAY_VALUES[i] =
                static_cast<RelayValue>(request->getParam(relayParam, POST_PARAM)->value().toInt());
        }
    }
...
void handleWifiSettings(AsyncWebServerRequest *request) {
    ...
    request->send(200, PLAIN_TEXT_CONTENT_TYPE,
                  "Wifi Name and Wifi Password updated. Restarting...");
...
void setupPreactPage() {
    server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
        AsyncWebServerResponse *response =
            request->beginResponse_P(200, HTML_CONTENT_TYPE, static_files::f_index_html_contents,
    ...
    for (int i = 0; i < static_files::num_of_files; i++) {
        server.on(static_files::files[i].path, [i](AsyncWebServerRequest *request) {
            AsyncWebServerResponse *response = request->beginResponse_P(
                200, static_files::files[i].type, static_files::files[i].contents,
    ...
    server.on(
        ...
        [](AsyncWebServerRequest *request) {
            AsyncWebServerResponse *response = request->beginResponse(
                200, PLAIN_TEXT_CONTENT_TYPE, (Update.hasError()) ? "FAIL" : "OK");
...
void setRule(AsyncWebServerRequest *request) {
    ...
    if (!request->hasParam("i", POST_PARAM) || !request->hasParam("v", POST_PARAM)) {
        request->send(404, JSON_CONTENT_TYPE,
                      buildJson({{"Error", String("Relay or rules not found")}}));
        return;
    }
...
void setRelayLabel(AsyncWebServerRequest *request) {
    ...
    if (!request->hasParam("i", POST_PARAM) || !request->hasParam("v", POST_PARAM)) {
        request->send(404, JSON_CONTENT_TYPE,
                      buildJson({{"Error", String("Relay or label not found")}}));
        return;
    }
...
void getRelayConfig(AsyncWebServerRequest *request) {
    ...
    for (int i = 0; i < RUNTIME_RELAY_COUNT; i++) {
        pins.add(RUNTIME_RELAY_PINS[i]);
        inv.add(RUNTIME_RELAY_IS_INVERTED[i]);
    }
    String out;
...
void addRelay(AsyncWebServerRequest *request) {
    ...
    bool inv = request->hasParam("inv", POST_PARAM)
                   ? (request->getParam("inv", POST_PARAM)->value().toInt() == 1)
                   : true;
    ...
    for (int i = 0; i < RUNTIME_RELAY_COUNT; i++) {
        if (RUNTIME_RELAY_PINS[i] == pin) {
            request->send(400, JSON_CONTENT_TYPE,
                          buildJson({{"Error", String("pin already used")}}));
            return;
        }
    }
...
void removeRelay(AsyncWebServerRequest *request) {
    ...
    if (request->hasParam("i", POST_PARAM))
        idx = request->getParam("i", POST_PARAM)->value().toInt();
